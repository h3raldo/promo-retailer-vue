...
export default {
	...
	methods: {
		...
		imageURLToName( url )
		{
			url = url.replace('https://res.cloudinary.com/promo-retailer/image/upload/logos/', '');
			if( url.includes('googleusercontent') ) url = 'google-image';
		},
		prettify( column ){
...
<template>
	...
	<div v-else>
		...
		<Tabs :labels="tabs" first="Totals">
			<template #Totals>

				<h3 class="bg-light p-3 mb-0">Totals</h3>
				...
				<h3 class="bg-light p-3">By Vendor</h3>
				...
				<div class="px-3">
					<table class="table table-striped">
						...
						<tbody>
						<tr v-for="(company) in report.result_grouped">
							<td>{{ company.name }}</td>
							<td>{{ company.count }}</td>
							<td>{{ utils.pricing.format(company.total) }}</td>
						</tr>
						</tbody>
					</table>
				</div>
								...
								<template v-for="(sheet, sheet_id) in decorator.sheets">
									...
									<tr v-for="(logo, logo_id) in sheet.logos">
										...
										<td>
											<div class="row">
												<div class="col-8">
													<a :href="logo_id" target="_blank">
														{{ imageURLToName(logo_id) }}
													</a>
													<details>
														<summary>POs</summary>
														<ul>
